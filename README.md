...
The standard library’s [strings package](https://pkg.go.dev/strings) provides many useful string-related functions. Here are some examples to give you a sense of the package.
...
```sh
 go run 052_string_fmt/string_format.go
struct1: {1 2}
struct2: {x:1 y:2}
...
func main() {
	...
	// A value is considered false if it’s the default value of a type, such as 0,
	// an empty string, nil pointer, etc.
...
func main() {
	...
	// Streaming reads from os.Readers like os.Stdin or
	// HTTP request bodies is done with json.Decoder.
...
func main() {
	...
	// NewPCG creates a new PCG source that requires
...
The built-in package [strconv](https://pkg.go.dev/strconv) provides the number parsing.
